...
use near_sdk::{
    ...
    serde_json::json, assert_one_yocto, ext_contract, Gas, PromiseResult, PromiseOrValue
};
...
impl Contract {
    ...
    pub fn new_default_meta(owner_id: ValidAccountId, vault_id: ValidAccountId) -> Self {
        Self::new(
            ...
            NFTContractMetadata {
                ...
                name: "Venix".to_string(),
                symbol: "Venix".to_string(),
    ...
    pub fn nft_mint(
        ...
        // self.transaction_add(token, receiver_id.to_string(), None);
    ...
    pub fn put_nft_series_price(&mut self, token_series_id: TokenSeriesId
        ...
        let mut collection: Option<i128> = None;
        match token_series_id.split(TOKEN_DELIMETER).collect::<Vec<&str>>().len() {
            1=> {
                    ...
                    collection = Some(token_series.collection.clone());
                    assert_eq!(
    ...
    pub fn nft_transfer(
        ...
        // self.transaction_add(token_id.clone(), receiver_id_str.clone(), None);
    }*/
    ...
    pub fn get_market(&self,
        ...
        limit: Option<u64>) -> Vec<MarketJson> {
    ...
    pub fn get_market_collection(&self, collection: i128,
        from_index: Option<U128>,
        limit: Option<u64>) -> Vec<MarketJson> {
            ...
            self.marketplace.iter().filter(|(_k, s)| s.collection == Some(collection.clone()))
            .skip(start_index as usize)
            .take(limit)
            .map(|(k, s)| MarketJson {
            token_series_id: k.to_string(),
    ...
    pub fn get_Best_sellers(&self) -> Vec<ProfileJson> {
        //self.profile.iter().filter(|(k, s)| s.sales > 0).is_sorted_by_key(|(k, s)| s.sales.abs());

        self.profile.iter().filter(|(_k, s)| s.sales > 0)
        .map(|(k, s)| ProfileJson {
            user_id: k.to_string(),
            name: s.name.to_string(),
            last_name: s.last_name.to_string(),
            email: s.email.to_string(),
            bio: s.bio.to_string(),
            website: s.website.to_string(),
            sales: s.sales,
        }).collect()
        /*self.transaction.iter().map(|(k, s)| TransactionJson {
            token_id: s.token_id,
            operations: s.operations,
            sales: s.sales,
            operation_history: s.operation_history.iter().map(|x| OperationHistory { owner_id: x.owner_id.clone(), price: x.price, date: x.date }).collect(),
        }).collect();*/
        /*self.transaction.iter()
        .map(|(k, s)| TransactionJson {
            token_id: s.token_id,
            operations: s.operations,
            sales: s.sales,
            operation_history: s.operation_history.iter().map(|x| OperationHistory {
                owner_id: x.owner_id,
                price: x.price,
                date: x.date,
            }).collect(),
        }).collect().is_sorted_by_key(|b| b.sales.abs())*/
    }
...
near_contract_standards::impl_non_fungible_token_Receiver!(Contract, tokens);
near_contract_standards::impl_non_fungible_token_Resolver!(Contract, tokens);*/
